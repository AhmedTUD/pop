...
# إعداد المنطقة الزمنية المحلية
try:
    from timezone_config import get_timezone_from_env, get_timezone_info
    LOCAL_TIMEZONE = get_timezone_from_env()
    timezone_info = get_timezone_info(LOCAL_TIMEZONE)
    print(f"🕐 Using timezone: {timezone_info['name']} ({timezone_info['offset_string']})")
except ImportError:
    # إعداد افتراضي إذا لم يكن ملف الإعدادات متاحاً
    LOCAL_TIMEZONE = timezone(timedelta(hours=3))  # السعودية
    print("🕐 Using default timezone: Saudi Arabia (UTC+3)")
...
def get_local_time_string():
    """الحصول على الوقت المحلي كنص منسق"""
...
def submit_data():
    ...
    try:
        ...
        while f'branch_{model_index}' in request.form:
            ...
            # Branch is automatically saved for this employee only
...
def export_excel():
    ...
    try:
        ...
        if not entries:
            flash('⚠️ لا توجد بيانات للتصدير مع الفلاتر المحددة')
            return redirect(url_for('admin_dashboard'))
        
        # تصدير محسن مع الصور (بدون رسالة "جاري تصدير")
        result = export_enhanced_excel_with_cloudinary(entries)
        
        if result['success']:
            if result['method'] == 'cloudinary':
                # إعادة توجيه إلى رابط Cloudinary مع رسالة نجاح
                flash(f'✅ تم تصدير {len(entries)} سجل بنجاح! التقرير المحسن جاهز للتحميل.')
                return redirect(result['url'])
            else:
                # إرسال الملف مباشرة مع رسالة نجاح
                flash(f'✅ تم تصدير {len(entries)} سجل بنجاح!')
                return send_file(
                    BytesIO(result['data']),
    ...
    try:
        ...
        if is_cloudinary_configured():
            ...
            if result['success']:
                ...
                flash(f'✅ تم تصدير {len(entries)} سجل بنجاح! التقرير البسيط جاهز للتحميل.')
                return redirect(result['url'])
            else:
...
def user_management():
    ...
    # Get users with their branches count
    c.execute('''SELECT u.*, 
                        (SELECT COUNT(*) FROM branches b WHERE b.employee_code = u.company_code) as branch_count,
                        (SELECT GROUP_CONCAT(b.branch_name, ', ') FROM branches b WHERE b.employee_code = u.company_code LIMIT 5) as sample_branches
                 FROM users u 
